...
- [x] **Step 1: 写 `ArticleSourceRegistry` 的失败测试**
...
- [x] **Step 5: 跑测试确认 GREEN，并在真机上对真实站点验证**
...
### Task C 执行记录（Step 1~5 已完成，Step 6~7 见下）

1. **样本来自当期真实抓取**（2026-09-24 经代理）：RSS `zoneid=1579` 返回 20 个 item；首篇即计划实测的 `7998765.html`（Wilbur and Orville Wright），全文 99 KB。缩减夹具保留全部结构特征（wsw 容器、wsw__embed 音频块、strong/em/a、分隔线段、h2、`&quot;`/`&#39;`/`&amp;` 实体），并写明来源与「不进 APK」声明。
2. **解析器真实健壮性坑**：样本**头注释里**就含有 `<div class="wsw">` 字面量，`indexOf` 首个命中是注释而非真容器——深度计数走到底无闭合直接失败（首跑抓到）。修法：**每个候选独立尝试**，无闭合就继续找下一个。这对真实页面同样必要（脚本/注释里可能出现同名字面量）。
3. **真实站点 description 带尾随空格**（实测），summary 落领域前必须 trim。
4. **变异测试**：把白名单 host 精确相等改成 `url.contains(host)` → `rejectsALookalikeHost` 真的变红。注意 `endsWith(host)` 对计划设计的 `…com.evil.test` 形态**恰好免疫**——该 lookalike 用例真正堵的是 contains 类写法；ends 后缀的漏洞形态（`evil-learningenglish.voanews.com`）由「精确相等 + https 前缀显式拆解」共同堵住。
5. **Step 5 真实站点验证**（一次性 JVM 验证跑完即删，结果落 `verification-logs/real-site-parse-2026-09-24.txt`）：RSS 20 items 全解析；99KB 真实页面提取**16 段/2802 字符**正文；无 mp3/播放器泄漏；无分隔线残留。
6. 三测试类**20/20 绿**；JVM 全量**55 类/323 用例/0 失败/0 跳过**（2026-09-24）。

- [x] **Step 6: 实现 `FetchArticleUseCase` 并补测试**
...
**Step 6 执行记录与偏差（2026-09-24）**：

1. **计划矛盾（关键）**：`ArticleQualityPolicy.validate` 对空译文是**三重拒绝**（`checkNotBlank("")`、`isNotChinese("")` 返回 true），抓取文章（`chineseText=""`）永远 `QualityRejected`——计划设计的 `fetchRejectsABodyThatFailsQuality` 在旧入口下是**空过**（任何正文都被拒）。修法：新增 `validateFetched(raw, length)` 入口（`requireTranslation=false`，跳过译文相关两项检查），标题/正文、长度、语言、危险标记等其余检查全部照走；`validate` 原 12 用例行为不变。`FetchArticleUseCase` 改用 `validateFetched`。
2. **计划外配套改动**：`AiHttpRequest` 增加 `method` 参数（默认 `"POST"`），transport 支持 GET（不设 `doOutput`、拒绝带 body，`instanceFollowRedirects=false` 沿用）；假服务器 route 增加 method 维度 + `sendsAGetRequestWithoutABodyWhenAskedTo` 用例。新建 `ArticleIdFactory`；`ArticleRepository` 加 `findBySourceUrl`（接口加方法击穿假实现的老规律：两处 Fake 同步补齐）。
3. **编译期两坑**：计划伪代码里的 `HOST_PREFIX_HTTPS` 常量不存在 → 直接 `https://` 字面量；嵌套 `when` 带 subject 的布尔分支（`matched.sourceId != …`）被编译器当作与 subject 比较 → 改 `if/else`。
4. **变异测试**：白名单第二道闸 `matched == null` 临时放行 → `fetchRefusesAFeedItemWhoseUrlIsNotWhitelisted` 真红（transport 无脚本响应即抛错）；恢复后全绿。
5. **回归**：JVM 全量 **56 类 / 336 用例 / 0 失败 / 0 跳过**；真机 `RoomArticleRepositoryTest` **7 例绿**（新增 `findsAStoredArticleByItsSourceUrl`，内存库实现，不触真实数据），用户库三文件 MD5 前后逐字一致。
6. 本计划文件曾有一段**竖排字符损坏**（Task C 执行记录 412~1379 行，每字符一行），本步修复为横排。
