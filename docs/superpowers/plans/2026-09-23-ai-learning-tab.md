...
6. **真机录屏暴露出一次眨眼「闪三下」的渲染缺陷，已修。** 这是本轮最有价值的一条，记录完整过程：
   - **现象**：把录屏逐帧量化后发现，一次眨眼里眼睛的暗像素序列是
     `292 → 51 → 571 → 571 → 125 → 349 → 462 → 617 …`（暗像素 = 亮度低于 200 的像素数）。
   - **先排除竞争假设**：我最初的判据是「与眼睛色距离 ≤ 90」，太依赖色彩，细线被 H.264 一压就整片洗掉，容易把「眼睛其实在」的帧算成 0，制造假象。**换成只看亮度的判据后不连续依然成立**，而且最暗像素从相邻帧的 108 跳到 **180**（几乎全白）——压缩做不到这一点，所以定性为真实渲染缺陷。
   - **根因**：`eyeOpenFactor` 略大于 `CLOSED_EYE_THRESHOLD(0.02)` 时，画的是高度不足 1px 的圆角矩形（几乎看不见）；`factor` 归零时才切成固定粗细的 `drawLine`（线粗 = 半宽 × 0.5 = 7.5px）。**闭眼那一刻眼睛反而弹粗**，随后 `factor` 越过 0.02 又变回细缝。
   - **修法**：把「闭眼厚度」并入纯几何——新增 `eyeHalfHeightFactor = max(eyeOpenFactor, 0.25f)`，绘制端只剩圆角矩形一条路径、去掉 `CLOSED_EYE_THRESHOLD` 分支与 `drawLine`。取 `0.25` 是因为它正好等于旧闭眼线的半厚，衔接处厚度连续。
   - **修前 / 修后（同一台设备、同一路径、各录一段）**：
     - 修前 4 次眨眼**全部**在睁开段有反向跳变 **223~233**；整体最暗像素的全局最小值为 **8**（眼睛真的没了）。
     - 修后 4 次眨眼**全部单调**（两个方向反向跳变均为 0）；整体最小值升到 **450**，闭合时稳定停在 455 且最暗值 118。
   - **新增 3 条 JVM 断言**（闭眼厚度不下穿下限 / 衔接处连续 / 闭合过程只减不增），并做**变异测试**证明它们不是恒真：把 `maxOf` 换成裸 `eyeOpenFactor` 后立刻 2 条变红，恢复后 12/12 绿。
   - **可复现**：`verification-logs/check_blink_monotonic.py <视频>` 直接输出每次眨眼的闭合/睁开波形与反向跳变值，修前修后可直接对跑。
   - 教训：**闭眼线的粗细原本不在纯几何里，它活在绘制端的一个分支，所以任何几何测试都够不着**——这也是它能潜伏下来的原因。把这类「绘制常量」收进纯函数，才是让它可断言的前提。
