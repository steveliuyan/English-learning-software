...
- **闭眼是「一条有厚度的线」，不是「零高度」。** 绘制半高的下限是 `EYE_CLOSED_THICKNESS_FACTOR = 0.25`（相对眼睛半宽），由几何纯函数给出；绘制端只有圆角矩形一条路径，不再按阈值切换成 `drawLine`。这条是修掉一个真机缺陷后补的：早期实现让绘制高度直接等于 `eyeOpenFactor`、并在 `factor <= 0.02` 时改用固定粗细的线，于是闭合途中眼睛先细到几乎看不见（暗像素 292 → 51，最暗像素 108 → **180**），归零瞬间又切成粗线（**571**），接着再细回去（125）——**一次眨眼闪三下**。真机录屏逐帧量化：修前 4 次眨眼在睁开段均有 223~233 的反向跳变；修后全部单调，闭眼时暗像素稳定在 455、最暗值 118（与睁眼时的 114~121 同级，是一条正常深度的线，不是变淡的缝）。
  - 这个缺陷**单看任何一帧截图都看不出来**（每帧单看都正常），语义树也断言不到，只有把整个闭合过程的波形拉出来看是否单调才暴露。因此「眨眼波形」被拆成三条 JVM 断言：闭眼厚度不下穿下限、衔接处厚度连续、闭合过程中只减不增。**去掉下限做变异测试时这三条会变红，证明它们不是恒真的假测试。**
- 注意上述缺陷之所以能长期不被发现，是因为**闭眼线的粗细原本根本没进入纯几何**——它活在绘制端的一个分支里，任何几何测试都够不着。把下限移进 `AiMascotGeometryPolicy` 才是让它可以被断言的前提。
